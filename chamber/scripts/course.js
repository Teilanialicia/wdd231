...
function getCourseCredits(course){
    ...
    const creditCount = course.reduce(
        (courseSum, currentCourse) => courseSum + currentCourse.credits, 0
    )
    return creditCount;
}

function displayCourseCredits(course){
    document.getElementById("credits").innerText = "Credits: " + getCourseCredits(course);
}
